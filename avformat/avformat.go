...
//}
//
//int ineterruptCallback_cgo(void* vo);
//
...
func (s *Stream) CodecParameters() *avcodec.Parameters {
	if s.CAVStream.codecpar == nil {
		return nil
	}
	return avcodec.NewParametersFromC(unsafe.Pointer(s.CAVStream.codecpar))
}
...
func (ctx *Context) SetFileName(fileName string) {
	...
	// var num = int(C.sizeOfAVFormatContextFilename)
	// logger.Debug("-----", num)
	C.av_strlcpy(&ctx.CAVFormatContext.filename[0], cFileName, 1024)
}
...
func (ctx *Context) OpenInput(fileName string, input *Input, options *avutil.Dictionary) error {
	...
	cHost := C.CString("HOST: live.center.cmvideo.cn\r\n")
	defer C.free(unsafe.Pointer(cFileName))
	...
	// cRtsp:=C.CString("rtsp_transport")
	// cSTimeout:=C.CString("stimeout")
	// cTimeout:=C.CString("timeout")
	// cTime:=C.CString("3000000")
	// defer C.free(unsafe.Pointer(cRtsp))
	// defer C.free(unsafe.Pointer(cSTimeout))
	// defer C.free(unsafe.Pointer(cTimeout))
	// defer C.free(unsafe.Pointer(cTime))
	// //设置tcp or udp，默认一般优先tcp再尝试udp/ m_bTcp ? "tcp" : "udp"
	// C.av_dict_set(cOptions,cRtsp ,cTime, 0)
	// C.av_dict_set(cOptions, cSTimeout, cTime, 0)
	// //设置udp\http超时3秒
	// C.av_dict_set(cOptions, cTimeout, cTime, 0)

	if (strings.Index(fileName, "live.hkprivate.cmvideo.cn")) != -1 {
		C.av_dict_set(cOptions, cHeaders, cHost, 0)
		//cFileName = C.CString("http://117.131.18.229:8088/wd_r1/fifa/cctv5hd/480264/cloud/index.m3u8?msisdn=123")
	}
...
func (ctx *Context) SetBeginTime() {
	ctx.BeginTime = C.time(nil)
}

//export ineterruptCallback
func ineterruptCallback(ctx unsafe.Pointer) int {

	ct := (*Context)(ctx)
	curTime := C.time(nil)
	//设置超时时间
	if curTime-ct.BeginTime >= 16 {
		//中止阻塞
...
func (ctx *Context) SetInterruptCallback() {
	//记录起始时间
	ctx.SetBeginTime()
	ctx.CAVFormatContext.interrupt_callback.opaque = unsafe.Pointer(ctx)
	ctx.CAVFormatContext.interrupt_callback.callback = C.Callback(C.ineterruptCallback_cgo)
}
