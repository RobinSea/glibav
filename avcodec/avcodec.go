...
func NewParametersFromC(cCtx unsafe.Pointer) *Parameters {
	return &Parameters{
		CAVCodecParameters: (*C.AVCodecParameters)(cCtx),
		OptionAccessor:     avutil.NewOptionAccessor(cCtx, false),
	}
}

func (ctx *Parameters) GetSampleRate() int {
	return int(ctx.CAVCodecParameters.sample_rate)
}

func (ctx *Parameters) GetFormat() avutil.SampleFormat {
	return avutil.SampleFormat(ctx.CAVCodecParameters.format)
}

func (ctx *Parameters) GetChannels() int {
	return int(ctx.CAVCodecParameters.channels)
}
